...
sequenceDiagram
    ...
    DB-->>AS: User entity
    Note over AS: BCrypt.matches()<br/>JwtUtils.generate()
    AS-->>AC: LoginResponse
...
sequenceDiagram
    ...
    Note over F: validateToken()<br/>set SecurityContext
    F->>CT: forward request
    CT->>S: getAccounts()
    S->>DB: SELECT *
    DB-->>S: Account rows
    S-->>CT: AccountResponse
    CT-->>C: 200 OK [accounts]
```
...
```mermaid
sequenceDiagram
    participant C as Client (ROLE_USER)
    participant F as JwtAuthFilter
    participant AC as AdminController
    participant US as UserService
    participant DB as H2 Database

    C->>F: GET /api/admin/users (Bearer <user_token>)
    Note over F: validateToken()<br/>set SecurityContext
    F->>AC: forward request
    Note over AC: ⚠️ VULN: no role check → BFLA
    AC->>US: getAllUsers()
    US->>DB: SELECT *
    DB-->>US: User rows
    US-->>AC: User list
    AC-->>C: 200 OK [all users]
```
...
|---|---|---|---|---|---|---|
| S-01 | JWT token forgery via weak HS256 secret (`secret`) | `JwtUtils`, `application.properties` | **High** | A2 Crypto Failures | 3.A2.1 | Implemented |
| S-02 | `alg: none` attack — strip signature, bypass validation | `JwtAuthenticationFilter`, `JwtUtils` | **High** | A7 Auth Failures | 3.A7.3 | Implemented |
| S-03 | Credential stuffing / brute-force on `/api/auth/login` | `AuthController`, `AuthService` | **High** | A7 Auth Failures | 3.A4.1, 3.A7.1 | Implemented |
| S-04 | Accepting expired JWT tokens (skip expiry check) | `JwtAuthenticationFilter` | **Medium** | A7 Auth Failures | 3.A7.3 | Implemented |
| S-05 | Plaintext/MD5 password storage on `/api/auth/login-v2` | `AuthService` | **Critical** | A2 Crypto Failures | 3.A2.2 | Implemented |
...
| Threat ID | Threat | Component | Risk | OWASP | Phase 3 Task | Status |
|---|---|---|---|---|---|---|
| T-TX-01 | Negative transfer amount reverses fund flow (steal money) | `TransactionService`, `TransferRequest` | **Critical** | A8 Integrity Failures | 3.A8.1 | Implemented |
| T-TX-02 | Zero-amount transfer pollutes audit trail | `TransactionService` | **Medium** | A8 Integrity Failures | 3.A8.1 | Implemented |
| T-TX-03 | Transfer from any source account (no ownership check) | `TransactionController`, `TransactionService` | **Critical** | A1 Broken Access Control | 3.A1.4 | Implemented |
| T-AC-01 | Role escalation via `PUT /api/admin/users/{id}/role` | `AdminController` | **High** | A1 Broken Access Control | 3.A1.3 | Implemented |
| T-DB-01 | SQL injection in account search modifies query logic | `AccountService` (JDBC) | **Critical** | A3 Injection | 3.A3.1 | Implemented |
| T-DB-02 | SQL injection in transaction filter | `TransactionService` | **High** | A3 Injection | 3.A3.2 | Implemented |
| T-SER-01 | Insecure deserialization via `/api/admin/import` | `AdminController` | **Critical** | A8 Integrity Failures | 3.A8.2 | Implemented |
...
| Threat ID | Threat | Component | Risk | OWASP | Phase 3 Task | Status |
|---|---|---|---|---|---|---|
| R-01 | No immutable audit log for financial transactions | `TransactionService` | **High** | A4 Insecure Design | — | Implemented (doc only) |
| R-02 | Sensitive data (passwords, account numbers) logged in plaintext | `AuthService`, `TransactionService`, `logback.xml` | **High** | A2 Crypto Failures | 3.A2.3 | Implemented |
| R-03 | No correlation ID / request tracing across services | All controllers | **Medium** | A4 Insecure Design | — | Implemented (doc only) |
| R-04 | Admin actions (role change, delete user) not logged | `AdminController` | **High** | A1 Broken Access Control | 3.A1.3 | Implemented |
...
| Threat ID | Threat | Component | Risk | OWASP | Phase 3 Task | Status |
|---|---|---|---|---|---|---|
| I-01 | IDOR — `GET /api/accounts/{id}` exposes any account | `AccountController`, `AccountService` | **High** | A1 Broken Access Control | 3.A1.1 | Implemented |
| I-02 | IDOR — `GET /api/transactions/{id}` exposes any transaction | `TransactionController`, `TransactionService` | **High** | A1 Broken Access Control | 3.A1.2 | Implemented |
| I-03 | IDOR — `GET /api/transactions?accountId=` leaks other users' history | `TransactionController` | **High** | A1 Broken Access Control | 3.A1.2 | Implemented |
| I-04 | Stack traces returned in 500 responses | `application.properties`, error handling | **Medium** | A5 Security Misconfig | 3.A5.1 | Implemented |
| I-05 | H2 console exposed with no auth at `/h2-console` | `SecurityConfig`, `application.properties` | **High** | A5 Security Misconfig | 3.A5.2 | Implemented |
| I-06 | SQL error details leaked in API responses | Error handler | **Medium** | A5 Security Misconfig | 3.A5.1 | Implemented |
| I-07 | CORS wildcard + credentials allows cross-origin data theft | `WebConfig` | **High** | A5 Security Misconfig | 3.A5.3 | Implemented |
| I-08 | Predictable sequential account numbers enable enumeration | `AccountService` | **Medium** | A4 Insecure Design | 3.A4.2 | Implemented |
| I-09 | SSRF — server fetches attacker-supplied URL, probes internal network | `AccountController` (webhook) | **High** | A10 SSRF | 3.A10.1 | Implemented |
| I-10 | Command injection in export leaks server filesystem | `AdminController` (export) | **Critical** | A3 Injection | 3.A3.3 | Implemented |
...
| Threat ID | Threat | Component | Risk | OWASP | Phase 3 Task | Status |
|---|---|---|---|---|---|---|
| D-01 | No rate limiting on `/api/auth/login` — enables brute-force | `AuthController`, `SecurityConfig` | **High** | A4 Insecure Design | 3.A4.1 | Implemented |
| D-02 | No account lockout after failed login attempts | `AuthService` | **High** | A7 Auth Failures | 3.A7.1 | Implemented |
| D-03 | Unbounded transaction queries — no pagination | `TransactionController` | **Medium** | A4 Insecure Design | — | Implemented (doc only) |
| D-04 | Insecure deserialization gadget chain can crash JVM | `AdminController` (import) | **High** | A8 Integrity Failures | 3.A8.2 | Implemented |
| D-05 | SSRF to internal services can exhaust connection pool | `AccountController` (webhook) | **Medium** | A10 SSRF | 3.A10.1 | Implemented |
...
| Threat ID | Threat | Component | Risk | OWASP | Phase 3 Task | Status |
|---|---|---|---|---|---|---|
| E-01 | BFLA — `GET /api/admin/users` accessible to `ROLE_USER` | `AdminController`, `SecurityConfig` | **Critical** | A1 Broken Access Control | 3.A1.3 | Implemented |
| E-02 | BFLA — `PUT /api/admin/users/{id}/role` allows self-promotion | `AdminController` | **Critical** | A1 Broken Access Control | 3.A1.3 | Implemented |
| E-03 | BFLA — `DELETE /api/admin/users/{id}` accessible to `ROLE_USER` | `AdminController` | **High** | A1 Broken Access Control | 3.A1.3 | Implemented |
| E-04 | JWT forgery with forged `ROLE_ADMIN` claim | `JwtUtils`, `JwtAuthenticationFilter` | **Critical** | A2 / A7 | 3.A2.1, 3.A7.3 | Implemented |
| E-05 | Weak password policy allows trivially guessable passwords | `RegisterRequest`, `AuthService` | **Medium** | A7 Auth Failures | 3.A7.2 | Implemented |
| E-06 | Long-lived JWT (24h) with no revocation — stolen token = persistent access | `application.properties`, `JwtUtils` | **High** | A4 Insecure Design | 3.A4.3 | Implemented |
